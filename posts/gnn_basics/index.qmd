...
### 1. Propagate
This function is reponsible for orchestrating the message-passing process. It takes edge index a.k.a adjacency matrix as [mandatory parameter]{style="color:cyan"}.
You can (and probably must) give feature matrix [x]{style="color:cyan"}. In addition, you can give any other necessary data for the later steps we will see.
You don't update/override this function, you just pass the necessary data to it to be further used in the next steps.
...
If you use feature_vec=x, you should use [feature_vec_i]{style="color:cyan"} and [feature_vec_j]{style="color:cyan"} in the later steps.
:::
...
For example, in the second example of propagate you give [norm]{style="color:cyan"} parameter, you can access it in the message function as norm.
...
For example you may use the default sum aggregation but before that you can weight the messages based on edge attributes.

It takes:

- [inputs]{style="color:cyan"} which is the messages created in the message function
- [index]{style="color:cyan"} which say target node each message belongs to

And whatever you want from the propagate function.

### 4. Update

This is the final step where you update the target node features using the aggregated messages. Depending on your architecture,
you might do nothing here and just return the aggregated messages such as when you add self-loops. Alternatively, you might add
the source node features to the aggregated messages or pass them through a neural network layer.
...
- [For GNN basics and also for headline image reference](https://distill.pub/2021/gnn-intro/)
- [Also a greate Youtube video for GNN basics from Stanford](https://www.youtube.com/playlist?list=PLoROMvodv4rOP-ImU-O1rYRg2RFxomvFp)