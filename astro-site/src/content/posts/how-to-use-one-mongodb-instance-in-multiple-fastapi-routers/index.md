...
It is a pretty simple class: given a connection string, it connects to the database and assigns the client and db to the class variables. We only have one db operation which is `get_users()` and it returns all users in the database without their id in the database. `db/__init__.py` is next:
...
That's all :) We will use this `db` object from `main.py` and `users.py`! So, `main.py` is next:
...
As you can see from line 3 we import the `db` object from the previous file we looked at. On the FastAPI `startup` event, we connect to the MongoDB database using `connect_to_database()` and we close the connection on `shutdown`. Now time to go to `users.py`.
