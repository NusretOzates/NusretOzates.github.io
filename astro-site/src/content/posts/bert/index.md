...
Using Tensorflow Hub, training or fine-tuning BERT models is very easy. In the following steps, I will show you how you can use a BERT model to detect toxicity in texts for the [Toxic Comment Classification Challenge](https://www.kaggle.com/c/jigsaw-toxic-comment-classification-challenge/overview). Download the train data first and create a Tensorflow dataset, then separate it as train and validation set using this code:

```python
import tensorflow as tf
dataset = tf.data.experimental.make_csv_dataset(
    "data/kaggle_toxic_comments/train.csv",
