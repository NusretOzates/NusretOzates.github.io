...
hₜ ∈ ℝⁿ is a hidden state at time t, c is a vector, generated from the sequence of the hidden states. f and q functions are some non-linear functions. As an example, f can be an LSTM and _q(h_1…,h_Tx )_ can be equal to _h_T_.
...
The decoder predicts the next word _y_t_, given context vector c and all other previously predicted words. Finally, the probability of the translated sentence y is calculated with:
...
where g is a non-linear function that outputs the probability of _y_t_ and _s_t_ is the hidden state of the RNN.
...
This article has written with Hasan Kemik.

I want to thank M Asgari C for reviewing of this article.
